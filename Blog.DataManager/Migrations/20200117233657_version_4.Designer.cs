// <auto-generated />
using System;
using Blog.DataManager.EFCore.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace Blog.DataManager.Migrations
{
    [DbContext(typeof(BlogDbContext))]
    [Migration("20200117233657_version_4")]
    partial class version_4
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.1")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Blog.Model.Author", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("City")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Job")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Password")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Username")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Authors");

                    b.HasData(
                        new
                        {
                            Key = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            Active = true,
                            City = "Burdur/Türkiye",
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 906, DateTimeKind.Utc).AddTicks(5058),
                            EmailAddress = "husnuisleyen@gmail.com",
                            FirstName = "Hüsnü",
                            Job = "Yazılım Geliştirici",
                            LastName = "İşleyen",
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 906, DateTimeKind.Utc).AddTicks(5803),
                            Password = "s6ypLHk+4OmxqbCl9fwETgUUDfM=",
                            Username = "isleyen"
                        });
                });

            modelBuilder.Entity("Blog.Model.AuthorInterestMapping", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("InterestKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("InterestKey");

                    b.ToTable("AuthorInterestMappings");

                    b.HasData(
                        new
                        {
                            Key = new Guid("33709aa3-7b1b-4f57-8712-8df107da56ce"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(2822),
                            InterestKey = new Guid("44c79cb5-432c-449c-916b-3d5ebd6d05e4"),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(2826)
                        },
                        new
                        {
                            Key = new Guid("cf831acd-cad3-4428-9282-5b23a4bd7ee8"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(3864),
                            InterestKey = new Guid("d439aa93-c0f6-44de-b375-7b6abc8bb1b7"),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(3867)
                        },
                        new
                        {
                            Key = new Guid("40e15090-ba71-4243-8610-16bddd85872a"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(3871),
                            InterestKey = new Guid("e5facd10-0cf5-419b-ab20-a2b016f96909"),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(3872)
                        });
                });

            modelBuilder.Entity("Blog.Model.Category", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid?>("ParentKey")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Key");

                    b.HasIndex("ParentKey");

                    b.ToTable("Categories");
                });

            modelBuilder.Entity("Blog.Model.Comment", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("Author")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("PostKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("State")
                        .HasColumnType("int");

                    b.HasKey("Key");

                    b.HasIndex("PostKey");

                    b.ToTable("Comments");
                });

            modelBuilder.Entity("Blog.Model.Company", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LogoUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Companies");

                    b.HasData(
                        new
                        {
                            Key = new Guid("a51cddcb-8896-4bd2-8446-18031b7f9255"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(6364),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(6372),
                            Location = "Denizli/Türkiye",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Bereket Enerji Üretim A.Ş."
                        },
                        new
                        {
                            Key = new Guid("a9da48ef-c0c0-4f52-8c40-7415609a6e65"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(6973),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(6977),
                            Location = "Denizli/Türkiye",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Netinternet"
                        });
                });

            modelBuilder.Entity("Blog.Model.Course", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CertificateUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Date")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Owner")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Place")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Topic")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("Courses");
                });

            modelBuilder.Entity("Blog.Model.Education", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Begin")
                        .HasColumnType("datetime2");

                    b.Property<string>("Branch")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Department")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("End")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<float?>("Score")
                        .HasColumnType("real");

                    b.Property<Guid>("UniversityKey")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("UniversityKey");

                    b.ToTable("Educations");

                    b.HasData(
                        new
                        {
                            Key = new Guid("29e5484d-3764-4bfe-b01d-7cd7e87c2039"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            Begin = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(6613),
                            Branch = "Bilgisayar Mühendisliği",
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(6023),
                            Department = "Mühendislik Fakültesi",
                            Detail = "",
                            End = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(7291),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(6029),
                            Score = 3.46f,
                            UniversityKey = new Guid("c0415b0b-6b62-49ca-ab2f-45dd08b617cb")
                        },
                        new
                        {
                            Key = new Guid("fb62fed3-95d7-4bc7-976a-4a860266f00e"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            Begin = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(390),
                            Branch = "Computer Science",
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(384),
                            Department = "Faculty of Automatic Control, Electronics and Computer Science",
                            Detail = "Erasmus+",
                            End = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(391),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(389),
                            UniversityKey = new Guid("c0415b0b-6b62-49ca-ab2f-45dd08b617cb")
                        });
                });

            modelBuilder.Entity("Blog.Model.Experience", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Begin")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("CompanyKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("End")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Position")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("CompanyKey");

                    b.ToTable("Experiences");

                    b.HasData(
                        new
                        {
                            Key = new Guid("a264dfae-ec73-4703-96c9-7b3504302914"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            Begin = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(9388),
                            CompanyKey = new Guid("a9da48ef-c0c0-4f52-8c40-7415609a6e65"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(9379),
                            End = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(748),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(9386),
                            Position = "Stajyer"
                        },
                        new
                        {
                            Key = new Guid("23972cfc-19dc-4bd2-b4f3-27de23370810"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            Begin = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(1550),
                            CompanyKey = new Guid("a9da48ef-c0c0-4f52-8c40-7415609a6e65"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(1543),
                            End = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(1552),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(1548),
                            Position = "Stajyer"
                        },
                        new
                        {
                            Key = new Guid("ee125f35-ca00-40a4-bb64-de9f4c52f255"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            Begin = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(1560),
                            CompanyKey = new Guid("a51cddcb-8896-4bd2-8446-18031b7f9255"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(1557),
                            End = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(1562),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(1559),
                            Position = "Yazılım Geliştirme Uzm.Yrd."
                        });
                });

            modelBuilder.Entity("Blog.Model.Interest", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Interests");

                    b.HasData(
                        new
                        {
                            Key = new Guid("44c79cb5-432c-449c-916b-3d5ebd6d05e4"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(1373),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(1379),
                            Name = "C#"
                        },
                        new
                        {
                            Key = new Guid("d439aa93-c0f6-44de-b375-7b6abc8bb1b7"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(1983),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(1988),
                            Name = "Python"
                        },
                        new
                        {
                            Key = new Guid("e5facd10-0cf5-419b-ab20-a2b016f96909"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(1993),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(1994),
                            Name = "ASP.NET Core"
                        });
                });

            modelBuilder.Entity("Blog.Model.Message", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Topic")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Messages");
                });

            modelBuilder.Entity("Blog.Model.Post", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CategoryKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<byte>("GaleryIndex")
                        .HasColumnType("tinyint");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("PrimaryImage")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("Publish")
                        .HasColumnType("datetime2");

                    b.Property<string>("ShortDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("State")
                        .HasColumnType("int");

                    b.Property<string>("Taxonomies")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ViewCounter")
                        .HasColumnType("int");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("CategoryKey");

                    b.ToTable("Posts");
                });

            modelBuilder.Entity("Blog.Model.Reference", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AccountUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("GSM")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Position")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("References");

                    b.HasData(
                        new
                        {
                            Key = new Guid("7abbdc2d-b2cf-49f1-8e31-9faee634f6be"),
                            AccountUrl = "https://visualgo.net/en",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 917, DateTimeKind.Utc).AddTicks(19),
                            FirstName = "Firstname",
                            GSM = "[ GIZLI ]",
                            ImageUrl = "https://via.placeholder.com/250",
                            LastName = "Lastname",
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 917, DateTimeKind.Utc).AddTicks(39),
                            Position = "SOFTWARE DEVELOPER"
                        },
                        new
                        {
                            Key = new Guid("5705caf2-7852-4cc0-befd-b2723ddf44a8"),
                            AccountUrl = "https://visualgo.net/en",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 917, DateTimeKind.Utc).AddTicks(4638),
                            FirstName = "Firstname2",
                            GSM = "[ GIZLI ]",
                            ImageUrl = "https://via.placeholder.com/250",
                            LastName = "Lastname2",
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 917, DateTimeKind.Utc).AddTicks(4644),
                            Position = "SOFTWARE DEVELOPER"
                        },
                        new
                        {
                            Key = new Guid("f25b1e57-9f12-4c27-9a8e-a5454787eec3"),
                            AccountUrl = "https://visualgo.net/en",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 917, DateTimeKind.Utc).AddTicks(4649),
                            FirstName = "Firstname3",
                            GSM = "[ GIZLI ]",
                            ImageUrl = "https://via.placeholder.com/250",
                            LastName = "Lastname3",
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 917, DateTimeKind.Utc).AddTicks(4650),
                            Position = "SOFTWARE DEVELOPER"
                        });
                });

            modelBuilder.Entity("Blog.Model.SocialPlatform", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AccountUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Logo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("SocialPlatforms");

                    b.HasData(
                        new
                        {
                            Key = new Guid("1e8c7ace-a228-47c9-9273-9d86650f869e"),
                            AccountUrl = "https://github.com/slyn",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3514),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3515),
                            Logo = "data:image/png;base64,",
                            Name = "github"
                        },
                        new
                        {
                            Key = new Guid("4cbe41da-c03e-41dc-98e3-3f6a97911552"),
                            AccountUrl = "https://twitter.com/_slyn",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3498),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3499),
                            Logo = "data:image/png;base64,",
                            Name = "twitter"
                        },
                        new
                        {
                            Key = new Guid("2e4f67c7-0921-4ae3-a6f9-1cc14080bed8"),
                            AccountUrl = "mailto:husnuisleyen@gmail.com",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(2061),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(2109),
                            Logo = "data:image/png;base64,",
                            Name = "google"
                        },
                        new
                        {
                            Key = new Guid("3ce24758-8a99-4f60-ba69-130a2e4bc3b3"),
                            AccountUrl = "https://medium.com/@husnuisleyen",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3475),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3485),
                            Logo = "data:image/png;base64,",
                            Name = "medium"
                        },
                        new
                        {
                            Key = new Guid("2be1a692-e8cb-4423-8c86-eb390e6d20af"),
                            AccountUrl = "https://www.linkedin.com/in/hüsnü-işleyen-29468475",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3508),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3510),
                            Logo = "data:image/png;base64,",
                            Name = "linkedin"
                        },
                        new
                        {
                            Key = new Guid("e908a827-0ced-4dd1-874d-a1c0f9e636d6"),
                            AccountUrl = "https://www.facebook.com/isleyenhusnu",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3492),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3493),
                            Logo = "data:image/png;base64,",
                            Name = "facebook"
                        },
                        new
                        {
                            Key = new Guid("fc108d39-3a3d-4ae3-a78e-af377b70475b"),
                            AccountUrl = "https://www.instagram.com/husnuisleyen/",
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3503),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 909, DateTimeKind.Utc).AddTicks(3504),
                            Logo = "data:image/png;base64,",
                            Name = "instagram"
                        });
                });

            modelBuilder.Entity("Blog.Model.Success", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("Date")
                        .HasColumnType("datetime2");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SuccessType")
                        .HasColumnType("int");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("Success");

                    b.HasData(
                        new
                        {
                            Key = new Guid("985aa1ac-f732-4435-8c17-bdabb6dcd59e"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(5252),
                            Date = new DateTime(2016, 6, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Detail = "3.46/4.0 - Bölüm İkinciliği",
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(5257),
                            Name = "Lisans Mezuniyet",
                            SuccessType = 1
                        },
                        new
                        {
                            Key = new Guid("cecec52f-3dbd-49c4-a1ca-ee234cbcc0b5"),
                            Active = true,
                            AuthorKey = new Guid("ea013573-7406-4c4c-b9dc-a479ec62d880"),
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(7658),
                            Date = new DateTime(2016, 5, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Detail = "UYGULAMA",
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 911, DateTimeKind.Utc).AddTicks(7663),
                            Name = "Lisans Tezi",
                            SuccessType = 2
                        });
                });

            modelBuilder.Entity("Blog.Model.University", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LogoUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Universities");

                    b.HasData(
                        new
                        {
                            Key = new Guid("c0415b0b-6b62-49ca-ab2f-45dd08b617cb"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(3184),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(3191),
                            Location = "Denizli/Türkiye",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Pamukkale Universitesi"
                        },
                        new
                        {
                            Key = new Guid("e8531f0d-439c-4aff-aceb-c9dda08ba3ec"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(4309),
                            LastUpdateDate = new DateTime(2020, 1, 17, 23, 36, 56, 910, DateTimeKind.Utc).AddTicks(4314),
                            Location = "Gliwice/POLAND",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Silesian University of Technology"
                        });
                });

            modelBuilder.Entity("Blog.Model.AuthorInterestMapping", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Interest", "Interest")
                        .WithMany()
                        .HasForeignKey("InterestKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Category", b =>
                {
                    b.HasOne("Blog.Model.Category", "ParentCategory")
                        .WithMany()
                        .HasForeignKey("ParentKey");
                });

            modelBuilder.Entity("Blog.Model.Comment", b =>
                {
                    b.HasOne("Blog.Model.Post", "Post")
                        .WithMany("Comments")
                        .HasForeignKey("PostKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Course", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Education", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.University", "University")
                        .WithMany()
                        .HasForeignKey("UniversityKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Experience", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Company", "Company")
                        .WithMany()
                        .HasForeignKey("CompanyKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Post", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany("Posts")
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Category", "Category")
                        .WithMany("Posts")
                        .HasForeignKey("CategoryKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Reference", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.SocialPlatform", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Success", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
