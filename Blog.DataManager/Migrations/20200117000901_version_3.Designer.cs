// <auto-generated />
using System;
using Blog.DataManager.EFCore.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace Blog.DataManager.Migrations
{
    [DbContext(typeof(BlogDbContext))]
    [Migration("20200117000901_version_3")]
    partial class version_3
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.1")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Blog.Model.Author", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("City")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Job")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Password")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Username")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Authors");

                    b.HasData(
                        new
                        {
                            Key = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            Active = true,
                            City = "Burdur/Türkiye",
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 773, DateTimeKind.Utc).AddTicks(8284),
                            EmailAddress = "husnuisleyen@gmail.com",
                            FirstName = "Hüsnü",
                            Job = "Yazılım Geliştirici",
                            LastName = "İşleyen",
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 773, DateTimeKind.Utc).AddTicks(8803),
                            Password = "s6ypLHk+4OmxqbCl9fwETgUUDfM=",
                            Username = "isleyen"
                        });
                });

            modelBuilder.Entity("Blog.Model.AuthorInterestMapping", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("InterestKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("InterestKey");

                    b.ToTable("AuthorInterestMappings");

                    b.HasData(
                        new
                        {
                            Key = new Guid("64ee839a-6e32-4928-ad1f-62cf5643ab47"),
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(4556),
                            InterestKey = new Guid("eb21e344-8a07-4f5e-8993-8334e4d36466"),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(4561)
                        },
                        new
                        {
                            Key = new Guid("fbe80c9d-5055-403f-9152-c7edd95233fb"),
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(5558),
                            InterestKey = new Guid("c104df0d-f8ce-413d-8d4d-f46ced65f705"),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(5561)
                        },
                        new
                        {
                            Key = new Guid("51d757e7-2669-461d-8dba-0d2991c1bef6"),
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(5564),
                            InterestKey = new Guid("bf22f649-0c8f-4204-b11c-67def9f26848"),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(5565)
                        });
                });

            modelBuilder.Entity("Blog.Model.Category", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid?>("ParentKey")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Key");

                    b.HasIndex("ParentKey");

                    b.ToTable("Categories");
                });

            modelBuilder.Entity("Blog.Model.Comment", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("Author")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("PostKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("State")
                        .HasColumnType("int");

                    b.HasKey("Key");

                    b.HasIndex("PostKey");

                    b.ToTable("Comments");
                });

            modelBuilder.Entity("Blog.Model.Company", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LogoUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Companies");

                    b.HasData(
                        new
                        {
                            Key = new Guid("eed44194-b902-45ce-8501-4ae96c3bc35d"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(2362),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(2367),
                            Location = "Denizli/Türkiye",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Bereket Enerji Üretim A.Ş."
                        },
                        new
                        {
                            Key = new Guid("a9fd69ec-e865-4f10-86d5-01c480e6e6a5"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(2782),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(2786),
                            Location = "Denizli/Türkiye",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Netinternet"
                        });
                });

            modelBuilder.Entity("Blog.Model.Course", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CertificateUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Date")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Owner")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Place")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Topic")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("Courses");
                });

            modelBuilder.Entity("Blog.Model.Education", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Begin")
                        .HasColumnType("datetime2");

                    b.Property<string>("Branch")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Department")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("End")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<float?>("Score")
                        .HasColumnType("real");

                    b.Property<Guid>("UniversityKey")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("UniversityKey");

                    b.ToTable("Educations");

                    b.HasData(
                        new
                        {
                            Key = new Guid("f19fbee9-51f2-4488-98b6-64557ec70739"),
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            Begin = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(9882),
                            Branch = "Bilgisayar Mühendisliği",
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(9422),
                            Department = "Mühendislik Fakültesi",
                            Detail = "",
                            End = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(333),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(9426),
                            Score = 3.46f,
                            UniversityKey = new Guid("5f627fea-b587-4334-adb9-7a5199e57fe4")
                        },
                        new
                        {
                            Key = new Guid("0b2867b1-8d29-48ec-acdb-1d39d120861d"),
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            Begin = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(2559),
                            Branch = "Computer Science",
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(2554),
                            Department = "Faculty of Automatic Control, Electronics and Computer Science",
                            Detail = "Erasmus+",
                            End = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(2561),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(2558),
                            UniversityKey = new Guid("5f627fea-b587-4334-adb9-7a5199e57fe4")
                        });
                });

            modelBuilder.Entity("Blog.Model.Experience", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Begin")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("CompanyKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("End")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Position")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("CompanyKey");

                    b.ToTable("Experiences");

                    b.HasData(
                        new
                        {
                            Key = new Guid("1c5e54f0-48df-4847-8959-6116582eb0e6"),
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            Begin = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(4604),
                            CompanyKey = new Guid("a9fd69ec-e865-4f10-86d5-01c480e6e6a5"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(4598),
                            End = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(5502),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(4603),
                            Position = "Stajyer"
                        },
                        new
                        {
                            Key = new Guid("e86f2fd7-b17e-47d0-984f-ea3eb9d9f1df"),
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            Begin = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(6111),
                            CompanyKey = new Guid("a9fd69ec-e865-4f10-86d5-01c480e6e6a5"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(6107),
                            End = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(6112),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(6110),
                            Position = "Stajyer"
                        },
                        new
                        {
                            Key = new Guid("e529c413-d099-4818-a266-36b74dd70274"),
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            Begin = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(6126),
                            CompanyKey = new Guid("eed44194-b902-45ce-8501-4ae96c3bc35d"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(6124),
                            End = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(6127),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(6125),
                            Position = "Yazılım Geliştirme Uzm.Yrd."
                        });
                });

            modelBuilder.Entity("Blog.Model.Interest", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Interests");

                    b.HasData(
                        new
                        {
                            Key = new Guid("eb21e344-8a07-4f5e-8993-8334e4d36466"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(3306),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(3312),
                            Name = "C#"
                        },
                        new
                        {
                            Key = new Guid("c104df0d-f8ce-413d-8d4d-f46ced65f705"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(3761),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(3766),
                            Name = "Python"
                        },
                        new
                        {
                            Key = new Guid("bf22f649-0c8f-4204-b11c-67def9f26848"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(3768),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 777, DateTimeKind.Utc).AddTicks(3769),
                            Name = "ASP.NET Core"
                        });
                });

            modelBuilder.Entity("Blog.Model.Message", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Topic")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Messages");
                });

            modelBuilder.Entity("Blog.Model.Post", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CategoryKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<byte>("GaleryIndex")
                        .HasColumnType("tinyint");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("PrimaryImage")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("Publish")
                        .HasColumnType("datetime2");

                    b.Property<string>("ShortDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("State")
                        .HasColumnType("int");

                    b.Property<string>("Taxonomies")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ViewCounter")
                        .HasColumnType("int");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("CategoryKey");

                    b.ToTable("Posts");
                });

            modelBuilder.Entity("Blog.Model.Reference", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AccountUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("GSM")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Position")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("References");
                });

            modelBuilder.Entity("Blog.Model.SocialPlatform", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AccountUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Logo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("SocialPlatforms");

                    b.HasData(
                        new
                        {
                            Key = new Guid("895e2601-a7fa-4205-8b22-44267cc532af"),
                            AccountUrl = "https://github.com/slyn",
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(520),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(521),
                            Logo = "data:image/png;base64,",
                            Name = "github"
                        },
                        new
                        {
                            Key = new Guid("9cee3371-54f6-4d40-9c72-9054e55e34fb"),
                            AccountUrl = "https://twitter.com/_slyn",
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(508),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(509),
                            Logo = "data:image/png;base64,",
                            Name = "twitter"
                        },
                        new
                        {
                            Key = new Guid("cc3146a1-83bd-4f87-9c46-a080e3cd6c0e"),
                            AccountUrl = "mailto:husnuisleyen@gmail.com",
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 775, DateTimeKind.Utc).AddTicks(9466),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 775, DateTimeKind.Utc).AddTicks(9495),
                            Logo = "data:image/png;base64,",
                            Name = "google"
                        },
                        new
                        {
                            Key = new Guid("ae1408be-7a22-4c2f-b597-ce74dc618e44"),
                            AccountUrl = "https://medium.com/@husnuisleyen",
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(482),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(486),
                            Logo = "data:image/png;base64,",
                            Name = "medium"
                        },
                        new
                        {
                            Key = new Guid("d720c349-0320-40e1-b242-8fcb5eb397a5"),
                            AccountUrl = "https://www.linkedin.com/in/hüsnü-işleyen-29468475",
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(517),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(518),
                            Logo = "data:image/png;base64,",
                            Name = "linkedin"
                        },
                        new
                        {
                            Key = new Guid("d6cd91dc-d960-48da-b426-469db75be969"),
                            AccountUrl = "https://www.facebook.com/isleyenhusnu",
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(491),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(492),
                            Logo = "data:image/png;base64,",
                            Name = "facebook"
                        },
                        new
                        {
                            Key = new Guid("f786181f-1bb5-461c-b96e-b1e87c2c6706"),
                            AccountUrl = "https://www.instagram.com/husnuisleyen/",
                            Active = true,
                            AuthorKey = new Guid("3e6fb570-ad6a-489f-8b6b-14e32292b064"),
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(512),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(512),
                            Logo = "data:image/png;base64,",
                            Name = "instagram"
                        });
                });

            modelBuilder.Entity("Blog.Model.University", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LogoUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Universities");

                    b.HasData(
                        new
                        {
                            Key = new Guid("5f627fea-b587-4334-adb9-7a5199e57fe4"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(7337),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(7341),
                            Location = "Denizli/Türkiye",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Pamukkale Universitesi"
                        },
                        new
                        {
                            Key = new Guid("335e80aa-557e-487f-9418-2a1b64626259"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(8156),
                            LastUpdateDate = new DateTime(2020, 1, 17, 0, 9, 0, 776, DateTimeKind.Utc).AddTicks(8159),
                            Location = "Gliwice/POLAND",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Silesian University of Technology"
                        });
                });

            modelBuilder.Entity("Blog.Model.AuthorInterestMapping", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Interest", "Interest")
                        .WithMany()
                        .HasForeignKey("InterestKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Category", b =>
                {
                    b.HasOne("Blog.Model.Category", "ParentCategory")
                        .WithMany()
                        .HasForeignKey("ParentKey");
                });

            modelBuilder.Entity("Blog.Model.Comment", b =>
                {
                    b.HasOne("Blog.Model.Post", "Post")
                        .WithMany("Comments")
                        .HasForeignKey("PostKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Course", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Education", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.University", "University")
                        .WithMany()
                        .HasForeignKey("UniversityKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Experience", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Company", "Company")
                        .WithMany()
                        .HasForeignKey("CompanyKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Post", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany("Posts")
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Category", "Category")
                        .WithMany("Posts")
                        .HasForeignKey("CategoryKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Reference", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.SocialPlatform", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
