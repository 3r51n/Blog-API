// <auto-generated />
using System;
using Blog.DataManager.EFCore.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace Blog.DataManager.Migrations
{
    [DbContext(typeof(BlogDbContext))]
    [Migration("20200116235251_version_2")]
    partial class version_2
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.1")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Blog.Model.Author", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("City")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Job")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Password")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Username")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Authors");

                    b.HasData(
                        new
                        {
                            Key = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            Active = true,
                            City = "Burdur/Türkiye",
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 707, DateTimeKind.Utc).AddTicks(371),
                            EmailAddress = "husnuisleyen@gmail.com",
                            FirstName = "Hüsnü",
                            Job = "Yazılım Geliştirici",
                            LastName = "İşleyen",
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 707, DateTimeKind.Utc).AddTicks(975),
                            Password = "s6ypLHk+4OmxqbCl9fwETgUUDfM=",
                            Username = "isleyen"
                        });
                });

            modelBuilder.Entity("Blog.Model.AuthorInterestMapping", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("InterestKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("InterestKey");

                    b.ToTable("AuthorInterestMappings");
                });

            modelBuilder.Entity("Blog.Model.Category", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid?>("ParentKey")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Key");

                    b.HasIndex("ParentKey");

                    b.ToTable("Categories");
                });

            modelBuilder.Entity("Blog.Model.Comment", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("Author")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("PostKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("State")
                        .HasColumnType("int");

                    b.HasKey("Key");

                    b.HasIndex("PostKey");

                    b.ToTable("Comments");
                });

            modelBuilder.Entity("Blog.Model.Company", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LogoUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Companies");

                    b.HasData(
                        new
                        {
                            Key = new Guid("2c98c0c0-316a-4c68-8be3-8bdf3fd50e85"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(7328),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(7335),
                            Location = "Denizli/Türkiye",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Bereket Enerji Üretim A.Ş."
                        },
                        new
                        {
                            Key = new Guid("6b31954d-8f0f-4a1b-b120-392ee5f08b5d"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(7937),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(7940),
                            Location = "Denizli/Türkiye",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Netinternet"
                        });
                });

            modelBuilder.Entity("Blog.Model.Course", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CertificateUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Date")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Owner")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Place")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Topic")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("Courses");
                });

            modelBuilder.Entity("Blog.Model.Education", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Begin")
                        .HasColumnType("datetime2");

                    b.Property<string>("Branch")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Department")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("End")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<float?>("Score")
                        .HasColumnType("real");

                    b.Property<Guid>("UniversityKey")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("UniversityKey");

                    b.ToTable("Educations");

                    b.HasData(
                        new
                        {
                            Key = new Guid("3de62a75-70ac-4d85-85b7-16322daed2b3"),
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            Begin = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(5540),
                            Branch = "Bilgisayar Mühendisliği",
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(5076),
                            Department = "Mühendislik Fakültesi",
                            Detail = "",
                            End = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(5998),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(5082),
                            Score = 3.46f,
                            UniversityKey = new Guid("bf196641-21d7-493b-bd83-821a6f1c8ff5")
                        },
                        new
                        {
                            Key = new Guid("fab169ca-c038-414a-9ab5-8d2ebdcede3c"),
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            Begin = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(8410),
                            Branch = "Computer Science",
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(8403),
                            Department = "Faculty of Automatic Control, Electronics and Computer Science",
                            Detail = "Erasmus+",
                            End = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(8411),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(8408),
                            UniversityKey = new Guid("bf196641-21d7-493b-bd83-821a6f1c8ff5")
                        });
                });

            modelBuilder.Entity("Blog.Model.Experience", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Begin")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("CompanyKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("End")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Position")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("CompanyKey");

                    b.ToTable("Experiences");

                    b.HasData(
                        new
                        {
                            Key = new Guid("a383ebe0-f1e5-4f26-b0db-a1f10102fae7"),
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            Begin = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(9897),
                            CompanyKey = new Guid("6b31954d-8f0f-4a1b-b120-392ee5f08b5d"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(9890),
                            End = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(880),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(9896),
                            Position = "Stajyer"
                        },
                        new
                        {
                            Key = new Guid("f2639288-30b7-425b-8a81-b4cc23328fce"),
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            Begin = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(1539),
                            CompanyKey = new Guid("6b31954d-8f0f-4a1b-b120-392ee5f08b5d"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(1535),
                            End = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(1541),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(1538),
                            Position = "Stajyer"
                        },
                        new
                        {
                            Key = new Guid("5eadba1b-6d90-4eb9-bbd4-91f01c730181"),
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            Begin = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(1546),
                            CompanyKey = new Guid("2c98c0c0-316a-4c68-8be3-8bdf3fd50e85"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(1544),
                            End = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(1547),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(1545),
                            Position = "Yazılım Geliştirme Uzm.Yrd."
                        });
                });

            modelBuilder.Entity("Blog.Model.Interest", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Interests");
                });

            modelBuilder.Entity("Blog.Model.Message", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Topic")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Messages");
                });

            modelBuilder.Entity("Blog.Model.Post", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CategoryKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<byte>("GaleryIndex")
                        .HasColumnType("tinyint");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("PrimaryImage")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("Publish")
                        .HasColumnType("datetime2");

                    b.Property<string>("ShortDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("State")
                        .HasColumnType("int");

                    b.Property<string>("Taxonomies")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ViewCounter")
                        .HasColumnType("int");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("CategoryKey");

                    b.ToTable("Posts");
                });

            modelBuilder.Entity("Blog.Model.Reference", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AccountUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("GSM")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Position")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("References");
                });

            modelBuilder.Entity("Blog.Model.SocialPlatform", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AccountUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Logo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("SocialPlatforms");

                    b.HasData(
                        new
                        {
                            Key = new Guid("654a61e7-0e4f-43b0-8873-695e6c460f02"),
                            AccountUrl = "https://github.com/slyn",
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5246),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5247),
                            Logo = "data:image/png;base64,",
                            Name = "github"
                        },
                        new
                        {
                            Key = new Guid("67e8e316-2d5e-4d1a-ae5c-830e70e430fd"),
                            AccountUrl = "https://twitter.com/_slyn",
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5235),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5236),
                            Logo = "data:image/png;base64,",
                            Name = "twitter"
                        },
                        new
                        {
                            Key = new Guid("1cb9ce91-6226-4eaa-b5a9-63d23fd4c420"),
                            AccountUrl = "mailto:husnuisleyen@gmail.com",
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(4307),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(4345),
                            Logo = "data:image/png;base64,",
                            Name = "google"
                        },
                        new
                        {
                            Key = new Guid("dfc6b9af-49f8-464a-8b4b-a31f10d4dab9"),
                            AccountUrl = "https://medium.com/@husnuisleyen",
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5221),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5226),
                            Logo = "data:image/png;base64,",
                            Name = "medium"
                        },
                        new
                        {
                            Key = new Guid("2925ece9-6fbb-4ffc-8108-23767a09d09d"),
                            AccountUrl = "https://www.linkedin.com/in/hüsnü-işleyen-29468475",
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5242),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5243),
                            Logo = "data:image/png;base64,",
                            Name = "linkedin"
                        },
                        new
                        {
                            Key = new Guid("6c9dbac7-9615-4333-bdcc-eb8ec8509621"),
                            AccountUrl = "https://www.facebook.com/isleyenhusnu",
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5231),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5232),
                            Logo = "data:image/png;base64,",
                            Name = "facebook"
                        },
                        new
                        {
                            Key = new Guid("a0e8cb42-be1f-4d5d-96c6-14d56200f495"),
                            AccountUrl = "https://www.instagram.com/husnuisleyen/",
                            Active = true,
                            AuthorKey = new Guid("53e71a75-8542-43e5-9766-9a6c09b32601"),
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5238),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 709, DateTimeKind.Utc).AddTicks(5239),
                            Logo = "data:image/png;base64,",
                            Name = "instagram"
                        });
                });

            modelBuilder.Entity("Blog.Model.University", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LogoUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Universities");

                    b.HasData(
                        new
                        {
                            Key = new Guid("bf196641-21d7-493b-bd83-821a6f1c8ff5"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(2867),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(2875),
                            Location = "Denizli/Türkiye",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Pamukkale Universitesi"
                        },
                        new
                        {
                            Key = new Guid("188b18bd-14f9-4f51-8eaa-a793c34fa717"),
                            Active = true,
                            CreateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(3699),
                            LastUpdateDate = new DateTime(2020, 1, 16, 23, 52, 50, 710, DateTimeKind.Utc).AddTicks(3703),
                            Location = "Gliwice/POLAND",
                            LogoUrl = "https://via.placeholder.com/250",
                            Name = "Silesian University of Technology"
                        });
                });

            modelBuilder.Entity("Blog.Model.AuthorInterestMapping", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Interest", "Interest")
                        .WithMany()
                        .HasForeignKey("InterestKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Category", b =>
                {
                    b.HasOne("Blog.Model.Category", "ParentCategory")
                        .WithMany()
                        .HasForeignKey("ParentKey");
                });

            modelBuilder.Entity("Blog.Model.Comment", b =>
                {
                    b.HasOne("Blog.Model.Post", "Post")
                        .WithMany("Comments")
                        .HasForeignKey("PostKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Course", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Education", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.University", "University")
                        .WithMany()
                        .HasForeignKey("UniversityKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Experience", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Company", "Company")
                        .WithMany()
                        .HasForeignKey("CompanyKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Post", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany("Posts")
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Category", "Category")
                        .WithMany("Posts")
                        .HasForeignKey("CategoryKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Reference", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.SocialPlatform", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
