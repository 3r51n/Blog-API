// <auto-generated />
using System;
using Blog.DataManager.EFCore.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace Blog.DataManager.Migrations
{
    [DbContext(typeof(BlogDbContext))]
    [Migration("20200115211804_Initial")]
    partial class Initial
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.1")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Blog.Model.Author", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("City")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Job")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Password")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Username")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Authors");

                    b.HasData(
                        new
                        {
                            Key = new Guid("44fe9185-2da4-4f7c-8265-9ea38c518472"),
                            Active = true,
                            City = "Burdur/Türkiye",
                            CreateDate = new DateTime(2020, 1, 15, 21, 18, 3, 490, DateTimeKind.Utc).AddTicks(1374),
                            EmailAddress = "husnuisleyen@gmail.com",
                            FirstName = "Hüsnü",
                            Job = "Yazılım Geliştirici",
                            LastName = "İşleyen",
                            LastUpdateDate = new DateTime(2020, 1, 15, 21, 18, 3, 490, DateTimeKind.Utc).AddTicks(1913),
                            Password = "s6ypLHk+4OmxqbCl9fwETgUUDfM=",
                            Username = "isleyen"
                        });
                });

            modelBuilder.Entity("Blog.Model.AuthorInterestMapping", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("InterestKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("InterestKey");

                    b.ToTable("AuthorInterestMappings");
                });

            modelBuilder.Entity("Blog.Model.Category", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid?>("ParentKey")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Key");

                    b.HasIndex("ParentKey");

                    b.ToTable("Categories");
                });

            modelBuilder.Entity("Blog.Model.Comment", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("Author")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("PostKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("State")
                        .HasColumnType("int");

                    b.HasKey("Key");

                    b.HasIndex("PostKey");

                    b.ToTable("Comments");
                });

            modelBuilder.Entity("Blog.Model.Company", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LogoUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Companies");
                });

            modelBuilder.Entity("Blog.Model.Course", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CertificateUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Date")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Owner")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Place")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Topic")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("Courses");
                });

            modelBuilder.Entity("Blog.Model.Education", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Begin")
                        .HasColumnType("datetime2");

                    b.Property<string>("Branch")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Department")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("End")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<float?>("Score")
                        .HasColumnType("real");

                    b.Property<Guid>("UniversityKey")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("UniversityKey");

                    b.ToTable("Educations");
                });

            modelBuilder.Entity("Blog.Model.Experience", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("Begin")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("CompanyKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Detail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("End")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Position")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("CompanyKey");

                    b.ToTable("Experiences");
                });

            modelBuilder.Entity("Blog.Model.Interest", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Interests");
                });

            modelBuilder.Entity("Blog.Model.Message", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Topic")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Messages");
                });

            modelBuilder.Entity("Blog.Model.Post", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CategoryKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<byte>("GaleryIndex")
                        .HasColumnType("tinyint");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("PrimaryImage")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("Publish")
                        .HasColumnType("datetime2");

                    b.Property<string>("ShortDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("State")
                        .HasColumnType("int");

                    b.Property<string>("Taxonomies")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ViewCounter")
                        .HasColumnType("int");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.HasIndex("CategoryKey");

                    b.ToTable("Posts");
                });

            modelBuilder.Entity("Blog.Model.Reference", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AccountUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("GSM")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Position")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("References");
                });

            modelBuilder.Entity("Blog.Model.SocialPlatform", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AccountUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<Guid>("AuthorKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Logo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.HasIndex("AuthorKey");

                    b.ToTable("SocialPlatforms");

                    b.HasData(
                        new
                        {
                            Key = new Guid("26a56741-e489-465b-81c9-6da1c2f8d74f"),
                            AccountUrl = "https://github.com/slyn",
                            Active = true,
                            AuthorKey = new Guid("44fe9185-2da4-4f7c-8265-9ea38c518472"),
                            CreateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4618),
                            LastUpdateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4618),
                            Logo = "data:image/png;base64,",
                            Name = "github"
                        },
                        new
                        {
                            Key = new Guid("6b6aee85-d02f-4b99-88c9-d9dcab4919c9"),
                            AccountUrl = "https://twitter.com/_slyn",
                            Active = true,
                            AuthorKey = new Guid("44fe9185-2da4-4f7c-8265-9ea38c518472"),
                            CreateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4593),
                            LastUpdateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4594),
                            Logo = "data:image/png;base64,",
                            Name = "twitter"
                        },
                        new
                        {
                            Key = new Guid("ad91a507-badd-4315-bcd2-f1dc55fcd99e"),
                            AccountUrl = "mailto:husnuisleyen@gmail.com",
                            Active = true,
                            AuthorKey = new Guid("44fe9185-2da4-4f7c-8265-9ea38c518472"),
                            CreateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(3609),
                            LastUpdateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(3644),
                            Logo = "data:image/png;base64,",
                            Name = "google"
                        },
                        new
                        {
                            Key = new Guid("9eabf7f5-fc18-4fa3-bedb-c2b307941cf3"),
                            AccountUrl = "https://medium.com/@husnuisleyen",
                            Active = true,
                            AuthorKey = new Guid("44fe9185-2da4-4f7c-8265-9ea38c518472"),
                            CreateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4582),
                            LastUpdateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4585),
                            Logo = "data:image/png;base64,",
                            Name = "medium"
                        },
                        new
                        {
                            Key = new Guid("1d238973-5d15-4b1c-96ca-d358cea7480a"),
                            AccountUrl = "https://www.linkedin.com/in/hüsnü-işleyen-29468475",
                            Active = true,
                            AuthorKey = new Guid("44fe9185-2da4-4f7c-8265-9ea38c518472"),
                            CreateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4614),
                            LastUpdateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4615),
                            Logo = "data:image/png;base64,",
                            Name = "linkedin"
                        },
                        new
                        {
                            Key = new Guid("98a9d408-3468-4194-9145-723d76722d48"),
                            AccountUrl = "https://www.facebook.com/isleyenhusnu",
                            Active = true,
                            AuthorKey = new Guid("44fe9185-2da4-4f7c-8265-9ea38c518472"),
                            CreateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4589),
                            LastUpdateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4590),
                            Logo = "data:image/png;base64,",
                            Name = "facebook"
                        },
                        new
                        {
                            Key = new Guid("bfeaf144-3072-4159-bdcb-cef9b1d99794"),
                            AccountUrl = "https://www.instagram.com/husnuisleyen/",
                            Active = true,
                            AuthorKey = new Guid("44fe9185-2da4-4f7c-8265-9ea38c518472"),
                            CreateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4596),
                            LastUpdateDate = new DateTime(2020, 1, 15, 21, 18, 3, 492, DateTimeKind.Utc).AddTicks(4597),
                            Logo = "data:image/png;base64,",
                            Name = "instagram"
                        });
                });

            modelBuilder.Entity("Blog.Model.University", b =>
                {
                    b.Property<Guid>("Key")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("Active")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("LastUpdateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Location")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LogoUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Key");

                    b.ToTable("Universities");
                });

            modelBuilder.Entity("Blog.Model.AuthorInterestMapping", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Interest", "Interest")
                        .WithMany()
                        .HasForeignKey("InterestKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Category", b =>
                {
                    b.HasOne("Blog.Model.Category", "ParentCategory")
                        .WithMany()
                        .HasForeignKey("ParentKey");
                });

            modelBuilder.Entity("Blog.Model.Comment", b =>
                {
                    b.HasOne("Blog.Model.Post", "Post")
                        .WithMany("Comments")
                        .HasForeignKey("PostKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Course", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Education", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.University", "University")
                        .WithMany()
                        .HasForeignKey("UniversityKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Experience", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Company", "Company")
                        .WithMany()
                        .HasForeignKey("CompanyKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Post", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany("Posts")
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Blog.Model.Category", "Category")
                        .WithMany("Posts")
                        .HasForeignKey("CategoryKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.Reference", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Blog.Model.SocialPlatform", b =>
                {
                    b.HasOne("Blog.Model.Author", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorKey")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
